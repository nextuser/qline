//# 高考单词
const gkQuery = "select * from stardict where tag like '%gk%' or tag like '%vk%' order by random() limit ${0};"
...
//#//#//# toefl 
const toeflQuery = "select * from stardict where not(tag like '%cet6%')  and not(tag like '%cet4%')  and tag like '%toefl%' order by random() limit ${0};"
...
const vkQuery = "select * from stardict where tag like '%vk%'  order by random() limit ${0};"

const tags = ['zk','gk','cet4','cet6','ky','toefl','gre','ielts','vk'];
