...
const REMOVE_SORE  = 96;// 成功2次可以完成练习
//最多需要连续成功3次。 最少连续成功2次. 事变一次，需要连续练习两次
const MAX_SCORE = 102;//不管错误多少次，最多因为错误需要增加1次成功
const SUC_STEP = -2;
const FAIL_STEP = 2;
class WordData {
...
class WordBook {
  ...
  studyWord(wordData, isCorrect) {
    ...
    // 更新分值
    node.score += isCorrect ? SUC_STEP : FAIL_STEP;
    // 分值不能超过 MAX_SCORE
    node.score = Math.min(node.score, MAX_SCORE);
