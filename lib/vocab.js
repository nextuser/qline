...
class VocabBook {
  ...
  async reset(){
    await this.ensureDbConected();
    this.db.run('DELETE FROM vocab',(result,err)=>{
      if(err) throw err;
  ...
  async getRandomWord() {
    ...
    getRandomWords(count = 10) {
      if(!this.db){
        console.log("db is not connected,call connect() first");
        return null;
      }
    ...
    return new Promise((resolve,reject) => {
      this.db.all(
        ...
        (err, rows) => {
          ...
          debug('find random word count',rows.length)
         
          const words = rows.map(row => row.word);
          debug("getRandom words return words:",words)
          resolve(words);
        }
      );
    });
